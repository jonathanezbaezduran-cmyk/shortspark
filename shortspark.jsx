...
function buildPrompt(niche, topic) {
  return `You are a YouTube Shorts viral content expert with deep knowledge of the 2026 algorithm, trending hooks, and what makes Shorts go viral.
...
function Spinner() {
  return (
    <svg width="20" height="20" viewBox="0 0 20 20" style={{ animation: "spin 0.8s linear infinite", display: "inline-block" }}>
      <circle cx="10" cy="10" r="8" fill="none" stroke="#0f172a" strokeWidth="2.5" strokeOpacity="0.3" />
      <path d="M10 2 A8 8 0 0 1 18 10" fill="none" stroke="#0f172a" strokeWidth="2.5" strokeLinecap="round" />
    </svg>
  );
}
...
export default function ShortSpark() {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          messages: [{ role: "user", content: buildPrompt(niche, t) }]
        })
      });
  ...
  const label = { fontSize: "0.68rem", color: "#475569", letterSpacing: "0.12em", marginBottom: "0.75rem", display: "block" };
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .ss-btn:hover { background: #38bdf8 !important; }
        .ss-btn:active { transform: scale(0.97); }
        .ss-title-row:hover { background: #1e293b !important; cursor: pointer; }
        textarea:focus, select:focus { outline: 1px solid #22d3ee !important; }
      ...
      <div style={{ maxWidth: "700px", margin: "0 auto" }}>
        ...
        <div style={{ ...card, marginBottom: "1rem", padding: "1.5rem" }}>
          <span style={label}>YOUR SHORTS TOPIC OR HOOK</span>
          <textarea
            value={topic}
            onChange={e => setTopic(e.target.value)}
            onKeyDown={e => { if (e.key === "Enter" && e.ctrlKey) analyze(); }}
            placeholder='e.g. "I let AI control my life for 24 hours and this happened"'
            rows={3}
            style={{
              width: "100%", boxSizing: "border-box", background: "#020817",
              border: "1px solid #1e293b", borderRadius: "10px", padding: "0.75rem 1rem",
              color: "#f1f5f9", fontSize: "0.9rem", resize: "vertical",
              fontFamily: "inherit", transition: "border 0.2s", lineHeight: "1.6"
            }}
          />
          <div style={{ display: "flex", gap: "0.75rem", marginTop: "1rem", alignItems: "flex-end" }}>
            <div style={{ flex: 1 }}>
              <span style={{ ...label, marginBottom: "0.4rem" }}>NICHE</span>
              <select
                value={niche}
            ...
            <button
              ...
              style={{
                background: topic.trim() && !loading ? "#22d3ee" : "#1e293b",
        ...
        {/* ─── ERROR ─── */}
        {error && (
          <div style={{ ...card, border: "1px solid rgba(239,68,68,0.35)", background: "rgba(239,68,68,0.06)", marginBottom: "1rem", color: "#f87171", fontSize: "0.85rem", textAlign: "center" }}>
            ⚠ {error}
          </div>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.85rem", animation: "fadeUp 0.5s ease" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "0.85rem" }}>
              <div style={{ ...card, textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                <span style={label}>VIRAL SCORE</span>
              ...
              <div style={card}>
                <span style={label}>KEY INSIGHT</span>
                <p style={{ color: "#e2e8f0", fontSize: "0.9rem", lineHeight: "1.65", marginBottom: "1rem" }}>{result.one_liner}</p>
                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>
                  {(() => {
                    const m = getStatusMeta(result.trending_status);
                    return (
                      <span style={{ fontSize: "0.72rem", padding: "4px 10px", borderRadius: "999px", background: m.bg, color: m.color, border: `1px solid ${m.border}`, fontWeight: "600" }}>
                        {m.label}
                      </span>
                    );
                  })()}
                  <span style={{ fontSize: "0.72rem", padding: "4px 10px", borderRadius: "999px", background: "rgba(255,255,255,0.04)", color: "#94a3b8", border: "1px solid #1e293b" }}>
                    Competition: {result.competition_level}
                  </span>
                  <span style={{ fontSize: "0.72rem", padding: "4px 10px", borderRadius: "999px", background: "rgba(34,211,238,0.08)", color: "#22d3ee", border: "1px solid rgba(34,211,238,0.2)" }}>
            ...
            {/* ROW 3: Titles */}
            <div style={card}>
              <span style={label}>⚡ OPTIMIZED TITLE VARIATIONS — click to copy</span>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                {result.title_variations.map((t, i) => (
                  <div
                    key={i}
                    className="ss-title-row"
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.85rem" }}>
              <div style={card}>
                <span style={label}>✅ STRENGTHS</span>
              ...
              <div style={card}>
                <span style={label}>⚠️ WEAKNESSES</span>
                <div style={{ display: "flex", flexDirection: "column", gap: "0.55rem" }}>
            ...
            {/* ROW 5: Improvements */}
            <div style={card}>
              <span style={label}>💡 HOW TO MAXIMIZE VIRAL REACH</span>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.55rem" }}>
                {result.improvements.map((imp, i) => (
                  <div key={i} style={{ display: "flex", gap: "0.75rem", alignItems: "flex-start", background: "#020817", border: "1px solid #1e293b", padding: "0.7rem 0.9rem", borderRadius: "8px" }}>
                    <span style={{ background: "#22d3ee", color: "#020817", borderRadius: "4px", padding: "0px 6px", fontSize: "0.68rem", fontWeight: "800", flexShrink: 0, lineHeight: "1.7" }}>{i+1}</span>
            ...
            {/* ROW 6: Best Times */}
            <div style={card}>
              <span style={label}>🕐 BEST POSTING WINDOWS</span>
              <div style={{ display: "flex", gap: "0.6rem", flexWrap: "wrap" }}>
                {result.best_times.map((t, i) => (
                  <span key={i} style={{ background: "#020817", border: "1px solid #1e293b", borderRadius: "8px", padding: "0.5rem 1rem", color: "#a78bfa", fontSize: "0.82rem" }}>{t}</span>
                ))}
              </div>
            </div>
            ...
            <div style={{ textAlign: "center", paddingTop: "0.5rem" }}>
              <p style={{ color: "#334155", fontSize: "0.7rem", letterSpacing: "0.1em" }}>SHORTSPARK · POWERED BY CLAUDE AI · ANALYZE AS MANY IDEAS AS YOU WANT</p>
            </div>
